  ...
  try {
    const totalData = await db.thongke.findOne({
      attributes: [
        ...
        [
          db.sequelize.fn("SUM", db.sequelize.col("TaiKhoanMoi")),
          "TongTaiKhoanMoi",
        ],
        [
          db.sequelize.fn("SUM", db.sequelize.col("BaiThiMoi")),
          "TongBaiThiMoi",
        ],
        [
          db.sequelize.fn("SUM", db.sequelize.col("SoLanLamBaiTheoThang")),
          "TongSoLanLamBaiTheoThang",
        ],
      ],
    });
    data.status = 200;
  ...
  try {
    const highestAverageScoreStudents = await db.ResultTest.findAll({
      attributes: [
        "MSV",
        [
          Sequelize.fn("ROUND", Sequelize.fn("AVG", Sequelize.col("Diem")), 1),
          "averageScore",
        ],
      ],
      group: ["MSV"],
